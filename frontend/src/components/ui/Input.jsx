...
const Input = React.forwardRef(({ className, type = "text", error, label, helperText, required, options = [], ...props }, ref) => {
    ...
    <div className="space-y-1">
      ...
      {type === "select" ? (
        ...
        >
          ...
          {options.map((opt, idx) =>
            typeof opt === "object"
              ? <option key={opt.value ?? opt.id ?? idx} value={opt.value ?? opt.id}>{opt.label ?? opt.nombre ?? opt.value ?? opt.id}</option>
