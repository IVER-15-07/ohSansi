...
class CrearTablaOpcionCampoTutor extends Migration
{
    ...
    public function up()
    {
        Schema::create('opcion_campo_tutor', function (Blueprint $table) {
            ...
            $table->foreignId('id_dependencia')->constrained('opcion_campo_tutor')->nullable()->onDelete('cascade');
            $table->foreignId('id_campo_tutor')->constrained('campo_tutor')->onDelete('cascade');

            $table->unique(['id_campo_tutor', 'valor', 'id_dependencia'], 'unique_id_olimpiada_campo_tutor');
        });
    }
